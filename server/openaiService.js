...
export const describeClothing = async (imagePath, weather_condition) => {
  ...
  try {
      ...
      messages: [
        {
          ...
          content: [
            {
              type: "text",
              text: `Describe the person's clothing in the image as a simple semi-colon-separated list. End the list with a natural, human-like comment on whether the outfit is appropriate for current temperature at ${weather_condition.temperature}, humidity at ${weather_condition.humidity}, weather condition at ${weather_condition.weather}, min temperature at ${weather_condition.min_temperature}, and max temperature at ${weather_condition.max_temperature} considering warmth and comfort. Example:
    ...
    const clothingItems = description
      .split(';') // Split by commas, "and", or new lines
